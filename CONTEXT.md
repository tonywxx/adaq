...
An immutable, User-reviewed Gate 11 decision that binds one exact Strategy Candidate Revision, its deterministic qualified Strategy Component Package with the Candidate's concrete parameter values frozen as singleton values, and the exact completed ADAQ-native Backtest and Validation evidence executed against that Package. Historical runs without that exact Package and Candidate binding cannot be reused. Package identity also records the generator version, SDK/ABI, toolchain, and canonicalization inputs; changing any of them creates a new Package and requires new evidence and Qualification. Snapshot, Universe, date windows, Risk Policy, Execution Profile, and Validation Protocol are separate Gate 11 evaluation inputs frozen in the evidence; changing them creates a new Qualification for the same Candidate Revision. Multiple accepted Qualifications may coexist for one Candidate Revision; Gate 12 must reference an exact Qualification identity and never infer the latest one. At least one successful chronological-holdout out-of-sample Validation Report is required; Selection and Final windows are disjoint, and sealed Final evidence is used for one qualification decision; other methods are additional evidence. Structural identity and evidence requirements fail closed; method-specific metric rules are frozen in the Validation Protocol rather than imposed as a universal profitability threshold. Failed attempts remain immutable evidence and reruns create new attempts; no partial attempt can produce or mutate a Qualification. Qualification is created only by explicit User action after server-side revalidation; rejection or no action leaves the evidence unqualified, with no V1 revocation or expiry semantics. All linked artifacts must belong to the same User scope; server-side ownership checks reject cross-user joins and cannot be replaced by frontend checks. The generator must support every currently valid Candidate operation and type; unsupported definitions fail closed without a manual-package or fallback path. Qualification grants eligibility as an accepted Gate 11 output for Gate 12 but does not authorize starting Gate 12; a separate User Workflow Continuation Approval is required. It grants no Paper, Risk, Execution, account, or Real Trading authority.
_Avoid_: Component Package conformance, Validation Report, Deployment Qualification, profitability guarantee
