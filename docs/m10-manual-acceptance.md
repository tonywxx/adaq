...
| Exact operation | Expected result | On failure, capture |
| --- | --- | --- |
| Run `cd src-tauri && cargo test -p adaq-feature-engine --release --test benchmarks -- --ignored --test-threads=1`. | Both canonical workloads complete serially: the 1,000,000-Bar Time-Series workload and the 10,000-Instrument × 252-Observation Cross-Sectional workload. The reviewed run completed both in a serial total of ~171.8 s: Time-Series 17,469 ms (cancellation stop 150 ms) and Cross-Sectional 58,076 ms, with a process peak RSS of 437,321,728 bytes. | Revision, complete output, workload, and platform. |
...
| Run `pnpm exec jest --watchman=false --runInBand`. | All frontend tests pass. The reviewed run passed 23 suites and 91 tests; the single prior failure was the M10 README-link contract test, resolved by the README updates in this step. | Revision, suite/test, and complete output. |
...
The #87 change contains documentation, README/roadmap, and frontend acceptance-contract tests only; it alters no Rust/provider, fixture, or packaging code, so the local gates above validate the reviewed revision while the recorded indicator-engine run remains the applicable cross-platform baseline and the recorded release run remains the packaging baseline for the unchanged release path. The Linux x86_64 matrix entry was introduced after that recorded baseline and no full three-platform run has yet succeeded; the full matrix will be evidenced by the next native (`src-tauri/**`) change, and until then Linux x86_64 is an explicitly recorded unresolved risk.
...
| P12 | Rust formatting/tests/checks, frontend Jest/build/lint, diff checks, accessibility review, and supported-platform CI pass for the final revision. | Gate table above; `src/loading-boundaries.test.ts` for accessibility-adjacent state. | Sections 11, 14. | macOS ARM64 and Windows x86_64 are covered by the recorded baseline run; the Linux x86_64 matrix entry was introduced after that baseline and remains an explicitly recorded unresolved risk until the next native (`src-tauri/**`) change evidences a full three-platform run. |
...
| M10.10 / #87 | This bilingual, cross-platform acceptance contract and evidence record. | `src/m10-manual-acceptance.test.ts`, all required gates. | Sections 1–15 and issue comment. | Linux x86_64 remains an explicitly recorded unresolved risk until a full three-platform run succeeds; all other rows are recorded. |

<!-- m10-acceptance:acceptance-record -->
## 16. Acceptance record and cleanup

Record the reviewed revision, OS/architecture/display scale, Node/pnpm/Rust versions, command outputs, focused test counts, full-gate conclusions, Dataset/Artifact/Attempt IDs, content-hash and reference-vector digests, revision and deletion-lock evidence, redacted User IDs, route screenshots, keyboard/accessibility observations, and the exact CI run URLs/SHA/conclusions. Also record the Linux x86_64 caveat: the matrix entry was introduced after the recorded baseline, so it stays an explicitly recorded unresolved risk until the next native (`src-tauri/**`) change evidences a full three-platform run. Keep credentials, tokens, private paths, and private market data out of the record.
