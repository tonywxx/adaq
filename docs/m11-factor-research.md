...
Status: accepted architecture and executable delivery baseline, published as [parent issue #88](https://github.com/tonywxx/adaq/issues/88). M10 is complete; no M11 production implementation is claimed by this document.
...
Dependencies are `#92 → #90 → #89 → #91 → #95 → #96`, `{#90,#89,#91,#95} → #94`, and `{#92,#90,#89,#91,#95,#96,#94} → #93`. #92 is the only initial executable frontier. Planning creates no production evidence; M11 remains unimplemented until its slices pass their own acceptance gates.
