...
| Following [External Kronos Adapter](../examples/external-models/kronos/README.md), acquire the pinned `Kronos-small` revision and record its licence and weight/config hashes. | The inference Model Artifact is exact and inspectable; no credentials are published. | Command, revision/URL, licence, HTTP error, size, and hashes. |
| Acquire the pinned `Kronos-Tokenizer-base` revision and record its licence and weight/config hashes. | The exact Tokenizer Artifact is available and is not described as the inference model. | Command, revision/URL, licence, HTTP error, size, and hashes. |
| Run the Adapter once against the exact section-3 Snapshot with Seed `7` and an explicitly selected CPU/MPS/CUDA device. | A Snapshot-aligned `kronos-small.adaq-signals` is produced. When hardware/network prevents this optional real-weight operation, retain the guide's complete unavailable evidence and do not claim it ran. | Runtime/config, Seed, device, peak memory, elapsed time, full traceback, and no credentials/private data. |
...
| Review README, SDK, Component, archive, Metric, external-model, and both manual-acceptance documentation entries listed in the root Documentation table. | Delivered M8 scope is consistent and none claims training, embedded Qlib/Python, Cross-sectional inference, live trading, Portfolio Optimization, OMS/EMS, Marketplace publishing, or future profitability. | File/link, quoted conflicting text, and expected scope statement. |
