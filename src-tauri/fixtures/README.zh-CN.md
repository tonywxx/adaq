...
这些示例涵盖了纯 Market（`strategy`）、External（`external-strategy`）、混合 Market/Built-in/External（`mixed-strategy`）、time-series 与 cross-sectional Factor ABI v2（`multi-output-factor`、`cross-sectional-factor`）以及重复 Factor 别名（`repeated-factor-strategy`）的 Feature Slot Plan。
