...
impl<'a> AttemptStore<'a> {
    ...
    pub(crate) fn retry_request(&self, attempt_id: &str, user_id: &str) -> Result<String, String> {
        self.database
            .query_row(
                "SELECT request_json FROM dataset_generation_attempts
             WHERE attempt_id = ?1 AND user_id = ?2
               AND status IN ('failed', 'cancelled')",
                params![attempt_id, user_id],
                |row| row.get(0),
            )
            .map_err(|_| "Dataset Generation Attempt cannot be retried".into())
    }
